...
int mem_est_alloue(int pByte){
    ...
    for(int i=0; i < memoryList->size; i++){
        ...
        if(currentMemBloc->start <= pByte && pByte < (currentMemBloc->start + currentMemBloc->length)){
            sprintf(debugMsg, "Byte : %d, is in the bloc memory at index %d with attributs : bloc type : %s, start = %d, length = %d", i,  getBlocTypeName(currentMemBloc->blocType), currentMemBloc->start, currentMemBloc->length);
            log(INFO, "allocationStrategy.c", debugMsg, __LINE__);
